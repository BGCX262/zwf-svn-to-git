 ...
 *
 * @version 2.2.1 20080428 JJ (Segundo)
 * @version 2.1.1 20080409 JJ (Segundo)
 * @version 2.1.0 20060501 AM (Segundo)
 ...
 * the same manner.
 * 
 * @version 2.1.1a (20080409 JJ)
 * @version 2.1.0d (20060501)
 * @package php.gt.goat
 */
class goat {
	...
	*/
	 ...
	 * @var boolean
	*/
	private $myStrict = GOAT_STRICT;
	/**
	 * String used as a tab by GOaT
	 * @access private
	 * @var string
	*/
	private $myTab = GOAT_DEFAULT_TAB;
	/**
	 * Default number of tabs GOaT prepends to the beginning of a new line
	 * @access private
	 * @var numeric
	*/
	private $myTabCount = GOAT_DEFAULT_TAB_COUNT;
	/**
	...
	*/
	public function generate_tag($gid, $cid, $strArgs = "", $tabCount = -1){
		/*
	...
	*/
	public function set_tab($strTab = GOAT_DEFAULT_TAB){
		$this->myTab = $strTab;
		return "";
	}
	...
	*/
	public function set_tab_count($tabCount = GOAT_DEFAULT_TAB_COUNT){
		if(is_int($tabCount)){
	...
	*/
	protected function register_error($msg){
		$err = array(
	 ...
	 * @return string
	*/
	public function pop_error($format = GOAT_TEMPLATE_ERROR_STD){
	 ...
	 * @return array
	*/
	public function pop_errors($format = GOAT_TEMPLATE_ERROR_STD){
		$retVal = array();
	...
	*/	
	 ...
	 * @return string
	*/
	public function clean_id($identifier){
	 ...
	 * @return string
	*/
	protected function build_exp($expType = GOAT_EXPTYPE_DEFAULT){
		switch($expType){
	...
	*/
	 ...
	 * @return Empty string
	*/
	public function register_error_callback($callback){
		if(is_callable($callback)){
			$this->myErrorCallback = $callback;
		}else{
			$this->register_error("Unable to define error callback; Invalid callback Id");
		}
	 ...
	 * @return boolean
	*/
	public function unregister_group($gid){
	 ...
	 * @return boolean
	*/
	public function callback_exists($gid, $cid){
		//ensure that all id's are safe and clean
		$gid = goat::clean_id($gid);
	 ...
	 * @return boolean
	*/
	public function variable_exists($vid){
		//ensure that all id's are safe and clean
	 ...
	 * @return boolean
	*/
	public function register_callback($gid, $cid, $callback){
		//ensure that all id's are safe and clean
		$gid = goat::clean_id($gid);
		$cid = goat::clean_id($cid);
	 ...
	 * @return boolean
	*/
	public function register_variable($vid, $value = ""){
		$vid = goat::clean_id($vid);
	 ...
	 * @return boolean
	*/
	public function mod_callback($gid, $cid, $callback){
		//ensure that all id's are safe and clean
		$gid = goat::clean_id($gid);
		$cid = goat::clean_id($cid);
	 ...
	 * @return boolean
	*/
	public function mod_variable($vid, $value){
		//ensure that all id's are safe and clean
		$vid = goat::clean_id($vid);
	 ...
	 * @return boolean
	*/
	public function unregister_callback($gid, $cid){
		//ensure that all id's are safe and clean
		$gid = goat::clean_id($gid);
		$cid = goat::clean_id($cid);
	 ...
	 * @return boolean
	*/
	public function unregister_variable($vid){
		//ensure that all id's are safe and clean
		$vid = goat::clean_id($vid);
	 ...
	 * @return string
	*/
	private function get_callback($gid, $cid){
		if($this->callback_exists($gid, $cid)){
	 ...
	 * @return string
	*/
	private function get_variable($vid){
		if($this->variable_exists($vid)){
	 ...
	 * @return mixed
	*/
	private function translate_arg($arg){
		if(is_string($arg)){
			$arg = trim($arg);
	 ...
	 * @return array
	 */
	protected function parse_arg_str($argStr){
	 ...
	 * @access public
	 */
	public function indent_output($subject, $tabs){
	 ...
	 * @return string
	*/
	private function handle_callback($matches){
	 ...
	 * @return string
	*/
	private function handle_variable($matches){
		//if we have too few captures, jump out and report an error
		if(count($matches) < 2){
	...
	public function parse_using_handle($subject, $handle, $exp_type){
		$i   = 0;
	 ...
	 * @return string
	*/
	public function parse($subject, $exp_type = GOAT_EXPTYPE_DEFAULT, $parse_ext = true){
